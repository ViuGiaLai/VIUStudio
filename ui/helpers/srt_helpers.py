...
def _shift_segment_timeline(
    ...
    *,
    ...
    """Add a timeline offset to every timing field on one cue, including words."""
    item = dict(segment)
...
def segments_to_source_video_time(segments: list | None, content_offset: float) -> list:
    """Convert timeline-relative cues back to source-video time for TTS mix.

    Voice WAVs are always mixed against the first video frame (t=0), never the
    intro image. UI/subtitle timestamps stay on the timeline clock.
    """
    if not segments:
        return []
    try:
...
def align_segments_to_video_start(
    ...
    ``first_video_start`` so a late first cue (speech after the intro length)
    is still shifted. Without ``force_offset``, an already timeline-relative
    track whose first cue starts at/after the video is left intact.
    Cues lying entirely inside the intro are dropped; overlapping onset is clamped.
    """
